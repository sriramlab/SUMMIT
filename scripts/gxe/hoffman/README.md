...
Options beginning with `--_gxe-` below are private deployment plumbing. They
...
"${GXE_SUMMIT}" \
  ...
  --env "${GXE_ENV}" \
  --covar "${GXE_COVAR}" \
  --_gxe-feature-cache "${GXE_CACHE}" \
  --_gxe-reference-shard \
  --_gxe-probe-offset "${GXE_PROBE_OFFSET}" \
  --nvecs 128 \
  --gxe-kernel-mode standardized \
  --gxe-genotype-scale sample \
  --write-gxe-jackknife \
...
"${GXE_SUMMIT}" \
  --_gxe-merge-shards "${GXE_PARTIAL_ROOT}/shard_00.gxe.shard.json" \
  --_gxe-feature-cache "${GXE_CACHE}" \
  --out "${GXE_B128_OUT}"
```
